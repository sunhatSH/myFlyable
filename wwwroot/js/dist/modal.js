...
class Modal {
    ...
    show(options) {
        ...
        this.isShowing = true;
        const { type = 'info', title = '', message = '', buttons = [], autoClose = false, autoCloseTime = 3000 } = options;
        if (!this.modal)
            return;
        // 设置弹窗类型
        ...
        if (buttons.length === 0) {
            ...
            if (defaultButton) defaultButton.textContent = '确定';
            ...
            buttons.forEach((button) => {
                ...
                if (btnElement) btnElement.textContent = button.text;
                btnElement.onclick = () => {
                    if (button.onClick) {
                        button.onClick();
                    }
    ...
    // 成功提示
    success(message, title = '成功', autoClose = true) {
        this.show({
            type: 'success',
            title,
            message,
    ...
    // 错误提示
    error(message, title = '错误', autoClose = false) {
        this.show({
            type: 'error',
            title,
            message,
    ...
    // 警告提示
    warning(message, title = '警告', autoClose = false) {
        this.show({
            type: 'warning',
            title,
            message,
    ...
    // 信息提示
    info(message, title = '提示', autoClose = true) {
        this.show({
            type: 'info',
            title,
            message,
...
// 全局快捷方法
window.showSuccess = (message, title, autoClose) => modal.success(message, title, autoClose);
window.showError = (message, title, autoClose) => modal.error(message, title, autoClose);
window.showWarning = (message, title, autoClose) => modal.warning(message, title, autoClose);
window.showInfo = (message, title, autoClose) => modal.info(message, title, autoClose);
window.showConfirm = (message, title, onConfirm, onCancel) => modal.confirm(message, title, onConfirm, onCancel);
window.showLoading = (message, title) => modal.loading(message, title);
...
window.confirmAsync = (message) => {
    return new Promise((resolve) => {
        modal.confirm(message, '确认', () => resolve(true), () => resolve(false));
    });
};
// 兼容原有的showMessage函数
