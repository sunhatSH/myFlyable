const API_BASE_URL = 'http://localhost:5181/api/users';

async function getVerifyCode(type) {
    try {
        const response = await fetch(`${API_BASE_URL}/hello_${type}`);
...
async function sendVerifyCode(email, username, guid, action) {
    try {
        const response = await fetch(`${API_BASE_URL}/send_verify_code?email=${encodeURIComponent(email)}&username=${encodeURIComponent(username)}&guid=${encodeURIComponent(guid)}&action=${action}`, {
            method: 'POST'
        });
